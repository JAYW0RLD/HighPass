...
> **중요 (2025 해커톤 업데이트):**
> 이제 **`@crypto.com/facilitator-client`** 공식 패키지 사용을 **강력 권장**합니다. 아래 수동 헤더 구현 방식은 참고용으로만 사용하고, 프로덕션 레벨에서는 반드시 공식 SDK를 사용하세요.

### 1. 인증 헤더 (X402 표준) - 공식 SDK 사용 시
...
// SDK가 자동으로 헤더를 생성하고 관리합니다. (상세 내용은 공식 문서 참조)
```

### 2. 수동 구현 (Legacy)
직접 구현해야 할 경우 아래 헤더를 포함해야 합니다:
